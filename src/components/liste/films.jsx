...
const [films, setFilms] = useState(list);
  ...
  return (
    <>
      ...
      <Link to="/ajouterfilm" className={style.favoriteButton}>
        Ajouter un film
      </Link>
      <Link to="/favoris" className={style.favoriteButton}>
        Voir mes favoris
      </Link>
      </div>
      {showAddFilmForm && <Ajouterfilm onAddFilm={handleAddFilm} />}
      {showFavorites && (
        <FavoritesList favorites={favorites} toggleFavorite={toggleFavorite} details={details} />
      )}
